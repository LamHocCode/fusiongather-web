  ...
  export default function DisplayMap(props: any) {
    ...
    useEffect(() => {
      ...
      searchBox.addListener("places_changed", () => {
        ...
        onSearchLocation({ coords: [place.geometry?.location.lng(), place.geometry?.location.lat()], location: place.formatted_address })
        map.fitBounds(place.geometry.viewport as google.maps.LatLngBounds);

        // Create a marker for searched location
        const newMarker = createMarker(map, place.geometry.location as google.maps.LatLng)
        markerArray.current.push(newMarker)
        markerArray.current[0].addListener("dragend", () => { // Fixed code
          const position = markerArray.current[0].getPosition() as google.maps.LatLng
    ...
    return (
      <div className="flex justify-center items-center h-[50vh]" ref={mapRef}>
      </div>
    );
  }
